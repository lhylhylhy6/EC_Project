...
void client_entry(void)
{
    ...
    // 此处修改为H7的ESP模块AP模式下的热点信息
    if (sendATCommand_Client("AT+CWJAP=\"ESP8266-AP2\",\"jetbotwyq\"\r\n", "OK", 10000) != HAL_OK)
    {
    ...
    // 此处改为H7的ESP模块AP模式下的IP，并对端口8086进行监听
    ...
    const char *weather_msg = "weather";

    if (sendATCommand_Client("weather\r\n", "SEND OK", 5000) != HAL_OK)
    {
