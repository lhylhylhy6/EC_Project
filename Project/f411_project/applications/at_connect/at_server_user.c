...
rt_err_t ESP8266_SetupAP(void) {
    ...
    if (sendATCommand_Server("AT+CWMODE=2\r\n", "OK", 3000) != HAL_OK)
    {
        rt_kprintf("ESP8266 set AP mode failed!\n");
        return;
    }

    if (sendATCommand_Server("AT+RST\r\n", "OK", 3000) != HAL_OK)
    {
    ...
    if (sendATCommand_Server(command, "OK", 3000) != HAL_OK)
    {
...
rt_err_t ESP8266_StartServer(void)
{
    ...
    if (sendATCommand_Server("AT+CIPMUX=1\r\n", "OK", 3000) != HAL_OK)
    {
    ...
    if (sendATCommand_Server("AT+CIFSR\r\n", "OK", 3000) != HAL_OK)
    {
    ...
    // 设置超时时间为1200s
    if (sendATCommand_Server("AT+CIPSTO=1200\r\n", "OK", 3000) != HAL_OK)
    {
...
void handle_tcp_server(void *param)
{
    ...
    while (1)
    {
        // 存在多个延时，如果等待时间过长会导致丢失接收，需要在客户端设置重确认机制
        if (sendATCommand_Server("", "+IPD,0,7:weather", 3000) == HAL_OK)
        {
//            rt_event_send(voice_event, EVENT_POSTURE);
            _uart_send_impl(3);
            rt_thread_mdelay(5000);
            rt_kprintf("received the weather flag!\n");
        }
//        else if(sendATCommand_Server("", "+IPD,1,11:fire_level1", 3000) == HAL_OK)
//        {
////            rt_event_send(voice_event, EVENT_FIRE_L1);
//            _uart_send_impl(1);
//            rt_thread_mdelay(5000);
//            rt_kprintf("received the fire_level1 flag!\n");
//        }
//        else if(sendATCommand_Server("", "+IPD,1,11:fire_level2", 3000) == HAL_OK)
//        {
////            rt_event_send(voice_event, EVENT_FIRE_L2);
//            _uart_send_impl(2);
//            rt_thread_mdelay(5000);
//            rt_kprintf("received the fire_level2 flag!\n");
//        }
//        else if(sendATCommand_Server("", "+IPD,1,11:fire_level2", 3000) == HAL_OK)
//        {
////            rt_event_send(voice_event, EVENT_FIRE_L2);
...
{
    rt_thread_t atserver_thread= rt_thread_create("esp8266_server_thread_init", at_server_entry, RT_NULL, 8196, 21, 1000);
    rt_thread_startup(atserver_thread);
    ESP8266_Server_Init();
}
//INIT_APP_EXPORT(esp8266_server_thread_init);
MSH_CMD_EXPORT(esp8266_server_thread_init,esp8266_server_thread_init);
