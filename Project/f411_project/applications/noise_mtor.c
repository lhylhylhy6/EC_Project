    ...
    while(1)
    {
        if(rt_pin_read(LM386_PIN) == PIN_HIGH)
        {
            ...
            else if(vol_index > 400)
            {
                ...
                if(mid_tick_cout > 3000)
                {
                    LOG_I("[%d][%d]The ambient noise is too loud!", tick_cout_1 = rt_tick_get(), vol_index);
...
void LM386_thread(void)
{
    ...
    tid = rt_thread_create("noise_thread", LM386_thread_entry, RT_NULL, 4096, 22, 1000);

    /* 如果获得线程控制块，启动这个线程 */
    if (tid != RT_NULL)
        rt_thread_startup(tid);
}
//MSH_CMD_EXPORT(LM386_thread,LM386_thread);
//INIT_APP_EXPORT(LM386_thread);
